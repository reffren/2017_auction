...
namespace Nigon.WebUI.Controllers
{
    ...
    public class SellerController : Controller
    {
        ...
        {
            int productViewId = productModel.productView.ProductViewID; // get id if product exists and we edited it

            repositoryProductView.SaveProductView(productModel.productView);

            if (productViewId == 0) //if productViewId==0 we will create new product
                productViewId = repositoryProductView.ProductViews.OrderByDescending(p => p.ProductViewID).Select(s => s.ProductViewID).First(); // after record data in table ProductViews get from that table - ProductViewID and record to the table Products (to match ProductViewID in both tables)
