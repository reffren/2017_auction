    ...
    public class CustomAccountProvider : MembershipProvider
    {
        ...
        public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            if (user == null)
            {
                ...
                using (var usersContext = new EFDbUserRepository())
                {
                    usersContext.AddUser(userObj);
                    var userInRole = usersContext.GetUser(username);  //copy UserID from Users to UsersInRoles
                    var getUserId = new UsersInRoles { UserID = userInRole.UserID, RoleID = 12 };
                    using (var usersInRolesContext = new EFDbUsersInRolesRepository())
                    {
        ...
        {
            var usersContext = new EFDbUserRepository();
